...
public class CommonResult {
	...
	public static <T> T formatToPojoBean(String jsonData, Class<T> clazz) 
	{
		CommonResult commonResult = formatToPojo(jsonData, clazz);
	 ...
	 */
	@SuppressWarnings("unchecked")
	public static <T> List<T> formatToListBean(String jsonData, Class<T> clazz) {
		CommonResult commonResult = formatToList(jsonData, clazz);
		if(commonResult!=null){
			if (commonResult.getStatus() == 200) {
				List<T> item = (List<T>) commonResult.getData();
				return item;
			}
		}
		return null;
