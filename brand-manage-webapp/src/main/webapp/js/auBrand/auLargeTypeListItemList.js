...
$(document).ready(function(){
	layui.use(['laydate', 'table', 'form', 'element', 'upload'], function() {
		...
		upload.render({
			elem : '#uploadExcel',
			url : appName+'/auLargeTypeItem/uploadExcel',
			accept : 'file',
			exts:'xls|xlsx',
			before:function(){
...
var loadFriendShipList = function(){
	table.render({
		...
		url : appName + '/auLargeTypeItem/auLargeTypeItemListData',
		width : '100%',
...
var addFriendShip = function(id) {
	var url = appName + "/auLargeTypeItem/addAuLargeTypeItemView";
	...
	var obj = {
		...
		yes: function(index, layero) {
			...
			iframeWin.laySubmit('#auLargeTypeItemForm');
		},
...
var delFriendShip = function(id){
	layer.confirm('你确定要删除吗?', {icon: 3, title:'提示', skin: 'layui-layer-molv'}, function(index){
       	saas.ajax({
			type: "post",
			url: "/auLargeTypeItem/delAuLargeTypeItem",
			data: {'id': id},
