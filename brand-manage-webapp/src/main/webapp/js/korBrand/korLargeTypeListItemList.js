...
$(document).ready(function(){
	layui.use(['laydate', 'table', 'form', 'element', 'upload'], function() {
		...
		table.on('tool(KorLargeTypeItemTable)', function(obj){
		    var data = obj.data;
		...
		upload.render({
			elem : '#uploadExcel',
			url : appName+'/korLargeTypeItem/uploadExcel',
			accept : 'file',
			exts:'xls|xlsx',
			before:function(){
...
var loadFriendShipList = function(){
	table.render({
		elem : '#KorLargeTypeItemTable',
		url : appName + '/korLargeTypeItem/korLargeTypeItemListData',
		width : '100%',
...
var searchFriendShip = function(){
	...
	layui.table.reload('KorLargeTypeItemTable', {
		url: appName + '/korLargeTypeItem/korLargeTypeItemListData',
		where: param,
		method:'post',
		done:function(){
...
var addFriendShip = function(id) {
	var url = appName + "/korLargeTypeItem/addKorLargeTypeItemView";
	...
	var obj = {
		...
		yes: function(index, layero) {
			...
			iframeWin.laySubmit('#korLargeTypeItemForm');
		},
...
var delFriendShip = function(id){
	layer.confirm('你确定要删除吗?', {icon: 3, title:'提示', skin: 'layui-layer-molv'}, function(index){
       	saas.ajax({
			type: "post",
			url: "/korLargeTypeItem/delKorLargeTypeItem",
			data: {'id': id},
