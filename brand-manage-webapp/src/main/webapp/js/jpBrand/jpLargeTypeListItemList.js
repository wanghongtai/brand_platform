...
$(document).ready(function(){
	layui.use(['laydate', 'table', 'form', 'element', 'upload'], function() {
		...
		table.on('tool(JpLargeTypeItemTable)', function(obj){
		    var data = obj.data;
		...
		upload.render({
			elem : '#uploadExcel',
			url : appName+'/jpLargeTypeItem/uploadExcel',
			accept : 'file',
			exts:'xls|xlsx',
			before:function(){
...
var loadFriendShipList = function(){
	table.render({
		elem : '#JpLargeTypeItemTable',
		url : appName + '/jpLargeTypeItem/jpLargeTypeItemListData',
		width : '100%',
...
var searchFriendShip = function(){
	...
	layui.table.reload('JpLargeTypeItemTable', {
		url: appName + '/jpLargeTypeItem/jpLargeTypeItemListData',
		where: param,
		method:'post',
		done:function(){
...
var addFriendShip = function(id) {
	var url = appName + "/jpLargeTypeItem/addJpLargeTypeItemView";
	...
	var obj = {
		...
		yes: function(index, layero) {
			...
			iframeWin.laySubmit('#jpLargeTypeItemForm');
		},
...
var delFriendShip = function(id){
	layer.confirm('你确定要删除吗?', {icon: 3, title:'提示', skin: 'layui-layer-molv'}, function(index){
       	saas.ajax({
			type: "post",
			url: "/jpLargeTypeItem/delEuroLargeTypeItem",
			data: {'id': id},
